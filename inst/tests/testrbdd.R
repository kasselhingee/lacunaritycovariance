...
test_that("rbdd produces simulations with the correct area fraction", {
  ...
  setcovB <- setcov(w)
  tryCatch(integrand <- eval.im((thcovariance-truecoveragefrac^2)*setcovB,harmonize = TRUE),
           warning = function(w) {
             if (!grepl("simpleWarning:  the images .thcovariance. and .setcovB. were not compatible\n", paste0(w))){
               warning(w)
             }
           })
...
test_that("Theoretical spectral density calculation is historically consistent", {
  ...
  expect_equal(thspecdens_origin, 64.41312)
