...
#' @param slots A list of names of slots to extract into a hyperframe. If not supplied then function will try to extract all slots a
...
#'      datadr::kvPair("fine",applyestimators(heather$fine,Frame(heather$fine))),
...
#' plot(hframe[,"sphcontactxi",drop=TRUE])
#' 
...
ddotohyperframe <- function(inddo,slots="all"){
  ...
  hframe <- hyperframe()
  for (slotn in slots){
    kvpairslist <- datadr::drLapply(inddo, function(v) v[[slotn]], combine=datadr::combCollect, params=list(slotn=slotn))  #returns a list of kvpairs
