context("fft calculations")

#test units using a sine wave
test_that("fft.im computes something like a dirac delta for sine wave input", {
...
#test if this peak is the centre pixel
expect_lt(abs(fftA$xcol[maxindex[2]]+1), fftA$xstep)
expect_lt(abs(fftA$yrow[maxindex[1]]-0), fftA$ystep)
expect_equal(as.numeric(fftA[3, 3]), 0) 
})

#test height using a Gaussian
test_that("fft.im result matches theoretical Gaussian value", {
...
expect_equal(max(Re(fftB)),  10*sqrt(pi/4), tolerance = 1E-8)
})
