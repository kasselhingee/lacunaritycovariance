...
test_that("im binary operations are performing correctly", {
  ...
  expect_error(Pmax(ims[[1]], ims[[2]]), regexp = "not compatible")
  expect_error(Pmin(ims[[1]], ims[[2]]), regexp = "not compatible")
  
  ims <- as.solist(do.call(harmonize.im, args = ims))

  expect_equal(sum(ims[[1]]) + sum(ims[[2]]) - sum(Pmax(ims[[1]], ims[[2]])), sum(Pmin(ims[[1]], ims[[2]])))
  
  expect_equal(sum(Add(ims[[1]], ims[[2]])), sum(ims[[1]]) + sum(ims[[2]]))
  expect_equal(sum(Square(Add(ims[[1]], ims[[2]]))), 34)
})
