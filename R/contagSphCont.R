...
#' 
#' @param Xi_H Conditional spherical contact distribution function for \eqn{\Xi}. 
#' Typically this is an \code{fv} object but could also be a vector of values.
#' In applications \code{Xi_H} would likely be estimated from an image using \code{\link{Hest}} in \pkg{spatstat}.
#' @param Xic_H Conditional spherical contact distribution for the complement of \eqn{\Xi}. 
...
#' @details Xi_H should be a function of radius that gives (or estimates) the probability of a disc of radius \eqn{r} not intersecting \eqn{\Xi} if the disc's centre is not in \eqn{\Xi} 
#' \deqn{\code{Xi_H}(r) = P(B_r(x) \subseteq \Xi^c | x \in \Xi^c).}
#' Similarly \code{Xic_H} should be an estimate of the probability of a disc being fully contained in \eqn{\Xi} given its centre is in \eqn{\Xi}
#' \deqn{\code{Xic_H}(r)\approx P(B_r(x) \subseteq \Xi | x \in \Xi).}
#' These can both be obtained using \code{\link{Hest}} in \pkg{spatstat}.
#' For \code{Xic_H} take care to apply Hest to the complement of \eqn{\Xi} with the observation window \eqn{W}.
#' 
...
#'
#' @return An \code{fv} object or a vector the same length as \code{Xi_H} corresponding to the contagion at each r value of \code{Xi_H}
...
#' xi_H <- Hest(xi,W=obswindow) #Sph. Contact Distrution Estimate
#' xic_H <- Hest(complement.owin(xi),W=obswindow) #Conditional Core Prob. Estimate
#' plot(xi.H,type="l",col="red") 
#' lines(xic.H,type="l",col="black") 
#' 
#' contagion <- scdcontagion(xi_H,xic_H,p)
#' plot(contagion)
#' 
#' @keywords spatial nonparametric 
scdcontagion <- function(Xi_H, Xic_H, p, normalise=FALSE){
  returnfv <- FALSE
  unitnames <- NULL
  if (is.fv(Xi_H) && is.fv(Xic_H)) {
    returnfv <- TRUE
    unitnames <- unitname(Xi_H)
    fvin <- list(Xi_H = Xi_H, Xic_H = Xic_H)
    Xi_Hf <- as.function.fv(Xi_H, value = ".y", extrapolate = TRUE)
    Xic_Hf <- as.function.fv(Xic_H, value = ".y", extrapolate = TRUE)
    argranges <- lapply(list(Xi_H = Xi_H, Xic_H = Xic_H), argumentrange)
    ...
    fvwlargestarg <- which.max(list(Xi_H = argranges$Xi_H[[2]],
                                   Xic_H = argranges$Xic_H[[2]]))
    xvals <- c(xvals,
  ...
  probofstate <- matrix(NA, nrow = 4, ncol = length(Xi_H))
  rownames(probofstate) <- c("P11", "P10", "P01", "P00")
  probofstate["P11", ] <- p * (1 - Xic_H)
  probofstate["P10", ] <- p * Xic_H
  probofstate["P00", ] <- (1 - p) * (1 - Xi_H)
  probofstate["P01", ] <- (1 - p) * (Xi_H)
