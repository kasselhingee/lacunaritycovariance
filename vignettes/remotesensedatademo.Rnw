...
For now \code{racssummfuncs} only has functions for estimating coverage probability, disc-state contagion, covariance and mass variance lacunarity, with the spherical contact distribution and conditional core probability estimated using \code{Hest} from \code{spatstat}.
 ...
 Below we estimate the coverage probability, spherical contact distribution, conditional core probability, disc-state contagion, covariance and mass variance lacunarity of the tree cover.
  ...
  Beware that other factors influence to validity of the estimates shorter distances may still produce erroneous results (e.g. if the observation window is much smaller than the spatial dependence)
...
paircorrest <- eval.fv(y / coverp ^ 2,
                       envir = list(y = rotmean(covar, padzero = FALSE, result = "fv")))
plot(paircorrest, main = "Estimated Isotropic Pair Correlation", xlim = c(0, 50))
@
\begin{center}
