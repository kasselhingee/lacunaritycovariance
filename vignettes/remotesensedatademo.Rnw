...
To use functions in \code{racssummfuncs} the observation must be converted into an \code{owin} object.
...
For now \code{racssummfuncs} only has functions for estimating coverage probability and disc-state contagion, with spherical contact distribution and conditional core probability estimated using \code{Hest} from \code{spatstat}.
...
discstatecontag <- contagSphCont(scdest,coreprob,coverp)
