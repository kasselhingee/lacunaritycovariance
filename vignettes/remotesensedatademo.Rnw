...
It is necessary to install the following packages to read remote sensing data and convert it into the formats used by \code{racstools}:
\begin{itemize}
\item sp
\item rgdal
\item maptools
...
<<readpolygon>>=
regionfilepath <- system.file("extdata", package = "racstools")
