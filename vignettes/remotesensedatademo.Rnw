...
It is necessary to install the following packages to read remote sensing data and convert it into the formats used by \code{racssummfuncs}:
\begin{itemize}
\item sp
\item rgdal
\item maptools
...
<<readpolygon>>=
regionfilepath <- system.file("extdata", package = "racssummfuncs")
...
For now \code{racssummfuncs} only has functions for estimating coverage probability, disc-state contagion, covariance and mass variance lacunarity, with the spherical contact distribution and conditional core probability estimated using \code{Hest} from \code{spatstat}.
 Conditional core probability is defined in Hingee (2016) as the expected proportion of any window that is core area, and is equivalent to the spherical contact distribution of the complement of the set.
 ...
 Covariance and mass variance lacunarity estimation and theory is described in Hingee (2018).
 
 Below we estimate the coverage probability, spherical contact distribution of the tree cover, the conditional core probability of the tree cover, disc-state contagion, covariance and mass variance lacunarity.
...
<<estimatecovariance, fig = TRUE, include = FALSE>>=
covar <- racscovariance(xiowin, obswin = rasterbdry)
plot(covar, main = "RACS Covariance Estimate", axes = TRUE)
@
\includegraphics[width=0.3\textwidth]{remotesensedatademo-racscovariance}

<<mvlestimation, fig = TRUE, include = FALSE>>=
mvlest <- mvlc(seq(1, 300/4, by = 1), covariance = covar, p = coverp)
plot(mvlest, main = "MVL Estimate")
@
\includegraphics[width=0.3\textwidth]{remotesensedatademo-mvlestimation}
