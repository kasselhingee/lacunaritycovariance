...
You can see how we prepared it from remote sensing data in the demo \verb!import_remote_sense_data!, which can be run by \verb!demo("import_remote_sense_data", package = "statracstools")!.

\begin{figure}[H]
\centering
<<loadbinarymap, fig = TRUE>>=
library(statracstools)
...
\begin{figure}[H]
\centering
<<mvlestimation, fig = TRUE>>=
mvlest <- mvl(xi = egbinarymap, seq(1, 200/4, by = 1))
