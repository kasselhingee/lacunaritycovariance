...
You can see how we prepared it from remote sensing data in the demo \verb!import_remote_sense_data!, which can be run by \verb!demo("import_remote_sense_data", package = "lacunaritycovariance")!.
...
For now \code{lacunaritycovariance} only has functions for estimating coverage probability, disc-state contagion, covariance and gliding box lacunarity, with the spherical contact distribution and conditional core probability estimated using \code{Hest} from \code{spatstat}.
