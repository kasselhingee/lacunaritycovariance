...
You can see how we prepared it from remote sensing data in the demo:\\
 \verb!import_remote_sense_data!.\\
This demo can be run by typing the following in \code{R}:\\ \verb!demo("import_remote_sense_data", package = "stationaryracsinference")!.
...
# the following converts egbinarymap to a thing of logical type
...
For now \code{stationaryracsinference} only has functions for estimating coverage probability, disc-state contagion, covariance and gliding box lacunarity, with the spherical contact distribution and conditional core probability estimated using \code{Hest} from \code{spatstat}.
 ...
 Covariance estimators are summarised in Hingee (2019).
 Theory and estimation of gliding box lacunarity is described in Hingee et al. (2017).
...
Ripley's rule of thumb for the K-function in spatial point processes suggests to estimate covariance for vectors $v$ no longer than 1/4 of the shortest width of the observation window.
...
This estimator has smaller variance than the traditional (plug-in moment) estimator of covariance from binary maps and is consistent with estimates made from the complement of the observed set. 
...
The additively modified estimate of pair-correlation (Picka, 2000) is shown below, along with the rotational mean of this estimate are shown below.
